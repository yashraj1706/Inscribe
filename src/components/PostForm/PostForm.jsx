  ...
  const submit = async (data) => {
    if (post) {
      const file = data.featuredImage[0]
        ? await databaseServiceObj.uploadFile(data.featuredImage[0])
        : null;
      if (file) {
        databaseServiceObj.deleteFile(post.featuredImage);
      }
      const dbPost = databaseServiceObj.updatePost(post.$id, {
        ...data,
      ...
      console.log("fileeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee",file)
      if (file) {
        console.log(data);
